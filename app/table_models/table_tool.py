...
class Tool(Base):
    ...
    purchase_date: Mapped[date| None] = mapped_column(Date, default=date.today) # дата покупки
    status: Mapped[StatusEnum] = mapped_column(SQLEnum(StatusEnum),
                                               default=StatusEnum.ACTIVE,
                                               nullable=False,
                                               )
    location_id: Mapped[int] = mapped_column(Integer, ForeignKey("locations.id"),default=1, nullable=False)
